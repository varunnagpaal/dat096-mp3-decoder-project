...
struct mp3_regs_t  * mp3_regs = (struct mp3_regs_t  *)0x80000900;

int flush_output(audio_output_t *ao, unsigned char *bytes, size_t count)
{
//Added C code to use DAC Core in realtime
	...
	countd=count;
  while(countd>0) {
	while(mp3_regs->flag_read_data && countd){
     		mp3_regs->send_data=data;
		countd=countd-2;
		a=*i;
		b=a;
		i++;
