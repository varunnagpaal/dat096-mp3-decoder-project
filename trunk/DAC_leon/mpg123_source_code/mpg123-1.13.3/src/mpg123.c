...
int play_frame(unsigned char *outputtest)
{
	unsigned char *audio;

	//Added C code to store decoded audio data to address indicated by *outputtest
	unsigned int countd,index;	//index is the amount of data stored in RAM
	unsigned char *i;
	index=0;	
...
//Added C code which stores decoded audio data into RAM  
	i=audio;	
	countd=bytes;
  while(countd>0) {
		countd=countd-1;
...
struct mp3_regs_t  * mp3_regs = (struct mp3_regs_t  *)0x80000c00;
...
//Added C code to store output in address starting from 0x40100000
	unsigned char *outputtest,*playaddress;  //starting address 
	//outputtest=1074790400;      //0x40100000 in hex
	outputtest=0x41800000;      
	int indexout;		    //stored data amount
	indexout=0;	
	int decode_start, list_end; //added to end program after decoding finished.
...
/* Rethink that SIGINT logic... */
		...
		while(!intflag)
		{
			...
			//Removed C code. New play_frame function gets address at which data will be stored.
			//if(!play_frame()) break;
			
			//Function returns the amount of stored data
			indexout=play_frame(outputtest);

			//If it is "0", it means last frame and program stops.
			//if(!play_frame()) break; this line changed as following to stop program after all decoding finished.
			if(!indexout) {
				if(decode_start==1) list_end=0;
				break;
			}
			else decode_start = 1;
			//if(!indexout) break;
			//the amount of stored data is added to address.
