...
int compat_open(const char *filename, int flags)
{
	...
	//added C code. MP3 files are stored at 0x41000000 as srec files. 
	int *ptr;
	int yuk;
	ptr=0x41000000;
	yuk=0;
...
fallback:
	...
	//added C code. Before mpg123 opens the audio file, it is created by following commands.
	ret =creat(filename ,S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
	yuk=write(ret,ptr,711294);
	close(ret);
